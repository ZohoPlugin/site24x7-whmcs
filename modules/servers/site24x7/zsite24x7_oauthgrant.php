...
if (strlen($code) > 0)
{
    ...
    try {
       $cli = Capsule::table('zoho_site24x7_auth_table')->where('region',$location)->first();
       if($location == 'cn') {
       ...
       $updatedUserCount = Capsule::table('zoho_site24x7_auth_table')
	...
	try {
		if (Capsule::schema()->hasTable('zoho_site24x7_auth_table'))
		{
      $client = Capsule::table('zoho_site24x7_auth_table')->first();
      if (strlen($client->clientId) > 3 ) {
        Capsule::table('zoho_site24x7_auth_table')->delete();
      }//do nothing
		} else 
		{
           Capsule::schema()->create(
                               'zoho_site24x7_auth_table',
                                function ($table) {
                                         $table->string('region');
    ...
    $statement = $pdo->prepare('insert into zoho_site24x7_auth_table (region, clientId, clientSecret, redirectUrl, token) values (:region, :clientId, :clientSecret, :redirectUrl, :token)');
    $statement->execute(
                           [
                            ':region' => $_POST['zm_dn'],
